...
public class MoterBikeFormController { 
    ...
    ObservableList<ItemDTO> allmoterBike= FXCollections.observableArrayList();
    ...
    private void LoadMoterBikeData() {
        allmoterBike.clear();
        try {
             allmoterBike = bo.getvehicle("MoterBike");
            tblmoterbike.setItems(allmoterBike);
        } catch (Exception ex) {
    ...
    private void Search() {
        FilteredList<ItemDTO> filteredListdata=new FilteredList<>(allmoterBike, e->true);
        txtsearch.setOnKeyReleased(e->{
            ...
            sortedList.comparatorProperty().bind(tblmoterbike.comparatorProperty());
            tblmoterbike.setItems(sortedList);
        });
